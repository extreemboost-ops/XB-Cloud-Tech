...
export default defineConfig(({ mode }) => {
  ...
  const siteConfiguration = readSiteConfiguration(path.resolve(process.cwd(), '.figma/make/site.json'))
...
function readSiteConfiguration(filePath: string): FigmaSiteConfiguration {
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? 'XB Cloud Tech'
